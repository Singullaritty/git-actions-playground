...
scan_result=$(curl -sSX POST --data-binary @"k8s_deployment.yaml" https://v2.kubesec.io/scan)
scan_message=$(curl -sSX POST --data-binary @"k8s_deployment.yaml" https://v2.kubesec.io/scan | jq .[0].message -r ) 
scan_score=$(curl -sSX POST --data-binary @"k8s_deployment.yaml" https://v2.kubesec.io/scan | jq .[0].score ) 
